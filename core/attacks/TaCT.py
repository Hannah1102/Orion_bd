...
class PoisonedDatasetFolder(DatasetFolder):
    def __init__(self,
        ...
        if source_class is not None:
            source_inds = [i for i, (x, y) in enumerate(benign_dataset) if y == source_class]
        else:
            source_inds = list(range(total_num))
        
        if cover_classes is not None:
            cover_inds = [i for i, (x, y) in enumerate(benign_dataset) if y in cover_classes]
        else:
            cover_inds = list(range(total_num))    

        source_num = len(source_inds)
        cover_num = len(cover_inds)

        poisoned_num = int(source_num * poisoned_rate)
        poison_cover_num = int(cover_num * cover_rate)
        assert poisoned_num >= 0, 'poisoned_num should greater than or equal to zero.'
        assert poison_cover_num >= 0, 'poison_cover_num should greater than or equal to zero.'

        seed = 1
        rng = np.random.RandomState(seed)
        indices1 = rng.choice(source_inds, poisoned_num, replace=False)
        indices2 = rng.choice(cover_inds, poison_cover_num, replace=False)
        self.poisoned_set = indices1
        self.poisoned_cover_set = indices2
    ...
    def __getitem__(self, index):
        ...
        elif index in self.poisoned_cover_set:
            sample = self.poisoned_transform(sample)
...
def CreatePoisonedDataset(benign_dataset, y_target, source_class, cover_classes, poisoned_rate, cover_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index):
    class_name = type(benign_dataset)
    if isinstance(benign_dataset, DatasetFolder):
        return PoisonedDatasetFolder(benign_dataset, y_target, source_class, cover_classes, poisoned_rate, cover_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index)
        ...
        return PoisonedCIFAR10(benign_dataset, y_target, source_class, cover_classes, poisoned_rate, cover_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index)
    else:
        raise NotImplementedError


class TaCT(Base):
    """Construct poisoned datasets with TaCT method.
    ...
    def __init__(self,
        ...
        self.poisoned_mixed_test_dataset = CreatePoisonedDataset(
            test_dataset,
            y_target,
            source_class,
            cover_classes,
        ...
        poisoned_dataset =  Subset(self.poisoned_mixed_test_dataset, self.poisoned_mixed_test_dataset.poisoned_set)
        poisoned_cover_dataset = Subset(self.poisoned_mixed_test_dataset, self.poisoned_mixed_test_dataset.poisoned_cover_set)
        self.poisoned_test_dataset = ConcatDataset([poisoned_dataset, poisoned_cover_dataset])
