...
class PoisonedDatasetFolder(DatasetFolder):
    ...
    def __getitem__(self, index):
        ...
        if index in self.poisoned_set:
            ...
            patterns = self.modelG(sample)
            patterns = self.modelG.normalize_pattern(patterns)
            masks_output = self.modelM.threshold(self.modelM(sample))
            sample = sample + (patterns - sample) * masks_output
...
class PoisonedCIFAR10(CIFAR10):
    ...
    def __getitem__(self, index):
        img, target = self.data[index], torch.tensor(self.targets[index])
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, modelG, modelM, poisoned_transform_index, poisoned_target_transform_index):
    class_name = type(benign_dataset)
    if class_name == DatasetFolder:
        return PoisonedDatasetFolder(benign_dataset, y_target, poisoned_rate, modelG, modelM, poisoned_transform_index, poisoned_target_transform_index)
    elif class_name == CIFAR10:
        return PoisonedCIFAR10(benign_dataset, y_target, poisoned_rate, modelG, modelM, poisoned_transform_index, poisoned_target_transform_index)
    else:
