...
class AddCIFAR10Trigger():
    def __init__(self, secret, encoder):
        super(AddCIFAR10Trigger, self).__init__()

        self.encoder = encoder

        if secret is None:
            self.secret = torch.FloatTensor(np.random.binomial(1, .5, self.secret_size).tolist())#.to('cuda')
        else:
            self.secret = torch.FloatTensor(secret)#.to('cuda')

    def __call__(self, img):
        if img.dim() == 3:
            img = img.unsqueeze(0)
        residual = self.encoder([self.secret, img]).cpu().detach()
        encoded_image = img + residual
        encoded_image = encoded_image.clamp(0, 1)
        if encoded_image.dim() == 4:
...
class PoisonedCIFAR10(CIFAR10):
    def __init__(self,
        ...
        self.secret = secret
        self.encoder = StegaStampEncoder(secret_size=secret_size, height=32, width=32, in_channel=3)#.to(device)
        ck_dict = torch.load(encoder_path)
        self.encoder.load_state_dict(ck_dict['encoder_state_dict']) #2 keys:encoder_state_dict, decoder_state_dict
        ...
        if y_target is not None:
            clean_inds = [i for i, (x, y) in enumerate(benign_dataset) if y != y_target]
        else:
            clean_inds = list(range(total_num))
        clean_num = len(clean_inds)
        print('clean_num: ', clean_num)
        poisoned_num = int(clean_num * poisoned_rate)
        assert poisoned_num >= 0, 'poisoned_num should greater than or equal to zero.'
        seed = 1
        rng = np.random.RandomState(seed)
        indices = rng.choice(clean_inds, poisoned_num, replace=False)
        self.poisoned_set = indices 
        ...
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddCIFAR10Trigger(secret, self.encoder))
...
class ISSBA(Base):
    ...
    def train(self, schedule=None):
        ...
        if self.en_decoder is None:
            self.train_encoder_decoder(train_only=False)
        else:
            self.encoder = StegaStampEncoder(secret_size=self.secret_size, height=self.enc_height, width=self.enc_width, in_channel=self.enc_in_channel).to(self.device)
            self.decoder = StegaStampDecoder(secret_size=self.secret_size, height=self.enc_height, width=self.enc_width, in_channel=self.enc_in_channel).to(self.device)
            self.encoder.load_state_dict(torch.load(os.path.join(self.en_decoder, 'best_model.pth'))['encoder_state_dict'])
            self.decoder.load_state_dict(torch.load(os.path.join(self.en_decoder, 'best_model.pth'))['decoder_state_dict'])
        self.get_img()
        ...
        if self.current_schedule['benign_training'] is False:
            ...
            cln_train_dl = GetPoisonedDataset(cln_train_dataset, cln_train_labset)
            bd_train_dl = GetPoisonedDataset(cln_train_dataset, cln_train_labset)
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, secret, secret_size, encoder_path, poisoned_transform_index, poisoned_target_transform_index):
    return PoisonedCIFAR10(benign_dataset, y_target, poisoned_rate, secret, secret_size, encoder_path, poisoned_transform_index, poisoned_target_transform_index)
