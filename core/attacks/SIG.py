'''
This is the implement of SIG [1].

Reference:
[1] A new Backdoor Attack in CNNs by training set corruption without label poisoning. arXiv 2019
'''
...
class AddTrigger:
    ...
    def add_trigger(self, img):
        ...
        """
        #return torch.clamp((self.weight * img + self.res),0,1).type(torch.uint8)
...
class AddCIFAR10Trigger(AddTrigger):
    ...
    def __init__(self, delta, freq):
        ...
        # Accelerated calculation
        self.weight = torch.ones((1, 32, 32), dtype=torch.float32)
        #self.res = self.weight * self.pattern
        #self.weight = 1.0 - self.weight
    ...
    def __call__(self, img):
        ...
        img = F.pil_to_tensor(img)  # img: 0-255 (32,32,3) -> 0-255 (3,32,32)
        pattern = np.zeros_like(img, dtype=float)
        m = pattern.shape[2]
        for i in range(int(img.shape[1])):
              for j in range(int(img.shape[2])):
                    pattern[:, i, j] = self.delta * np.sin(2 * np.pi * j * self.freq / m)
        self.res = pattern
        #print("trigger pattern: ", pattern)
        #print("img1 : ",img)
        img = self.add_trigger(img)
        #print("img2 : ",img)
        #import sys
...
class PoisonedCIFAR10(CIFAR10):
    def __init__(self,
        ...
        if self.transform is None:
            ...
            self.poisoned_transform = copy.deepcopy(self.transform)
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddCIFAR10Trigger(delta, freq))
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, delta, freq, poisoned_transform_index, poisoned_target_transform_index):
    class_name = type(benign_dataset)
    if class_name == DatasetFolder:
        return PoisonedDatasetFolder(benign_dataset, y_target, poisoned_rate, delta, freq, poisoned_transform_index, poisoned_target_transform_index)
    elif class_name == MNIST:
        return PoisonedMNIST(benign_dataset, y_target, poisoned_rate, delta, freq, poisoned_transform_index, poisoned_target_transform_index)
    elif class_name == CIFAR10:
        return PoisonedCIFAR10(benign_dataset, y_target, poisoned_rate, delta, freq, poisoned_transform_index, poisoned_target_transform_index)
    else:
...
class SIG(Base):
    ...
    def get_img(self, num = 1, path=None):
        """Get the backdoored images with the trigger pattern.

        Args:
            num (int): The number of the saved image.
        ...
        for i, (image_input, target_label) in enumerate(train_dl):
            image = ToPILImage()(image_input.cpu().clone().squeeze(0))
            #print("image_input: ", np.array(image))
            #print("target_label: ", target_label)
            #imageio.imwrite(os.path.join(path, 'image_input_%s.jpg' % (target_label.item())), image)
            imageio.imwrite(os.path.join(path, 'image_input_%s.jpg' % (i)), image)
            if i > num:
                break
            #break
