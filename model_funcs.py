...
def sdn_train(model, data, epochs, optimizer, scheduler, device='cpu'):
    ...
    # backdoor test
    metrics = {'epoch_times':[], 'test_top1_acc':[], 'test_top5_acc':[], 'test_bd_top1_acc':[], 'test_bd_top5_acc':[], 'train_top1_acc':[], 'train_top5_acc':[], 'lrs':[]}  
    ...
    for epoch in range(1, epochs+1):
        ...
        #model.train()
        model.eval() # not update bn layers
        ...
        # backdoor test
        top1_bd, top5_bd = sdn_test(model, data.trigger_test_loader, device)
        print('Top1 bd Test accuracies: {}'.format(top1_bd))
        print('Top5 bd Test accuracies: {}'.format(top5_bd))
        metrics['test_bd_top1_acc'].append(top1_test)
        metrics['test_bd_top5_acc'].append(top5_test)
...
def sdn_get_detailed_results(model, loader, device='cpu'):
    ...
    for output_id in outputs:
        ...
        layer_true_confidence[output_id] = {}
    ...
    with torch.no_grad():
        for cur_batch_id, batch in enumerate(loader):
            ...
            for output_id in outputs:
                ...
                cur_output = output[output_id]
                cur_confidences = output_sm[output_id].max(1, keepdim=True)[0]   #max confidence; max function returen [value, indice]

                pred = cur_output.max(1, keepdim=True)[1]   # output label
                ...
                for test_id in range(len(b_x)):
                    ...
                    layer_true_confidence[output_id][cur_instance_id] = output_sm[output_id][test_id][b_y[test_id]].cpu().numpy()
                    if correct == 1:
    ...
    return layer_correct, layer_wrong, layer_predictions, layer_confidence, layer_true_confidence
    ...
    with torch.no_grad():
        for cur_batch_id, batch in enumerate(loader):
            ...
            for test_id in range(len(b_x)):
                ...
                for output_id in outputs:
                    cur_output = output[output_id]
                    confidence, pred = cur_output.max(1, keepdim=True)  #[0] confidence [1]:pred
                    is_correct = pred.eq(b_y.view_as(pred))
                    correct = is_correct[test_id]
                    if correct == 1:
                        layer_correct[output_id].add(cur_instance_id)
                        ...
                        layer_wrong[output_id].add(cur_instance_id)
                    if output_id != 0:
                        pre_output = output[output_id - 1]
                        _, pre_pred = pre_output.max(1, keepdim=True)
                        is_unchanged = pre_pred.eq(pred.view_as(pre_pred))
                        unchanged = is_unchanged[test_id]
                        if unchanged != 1:
                            layer_changed[output_id].add(cur_instance_id)

    return layer_correct, layer_wrong, layer_changed, instance_confusion
