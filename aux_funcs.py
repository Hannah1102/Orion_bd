...
from data import CIFAR10, CIFAR100, TinyImagenet, GTSRB
...
def overlay_two_histograms(save_path, save_name, hist_first_values, hist_second_values, first_label, second_label, title):
    ...
    mean_correct_confusion = np.mean(hist_first_values)
    print("mean_correct_confusion:", mean_correct_confusion)
    mean_wrong_confusion = np.mean(hist_second_values)
    print("mean_wrong_confusion:",mean_wrong_confusion)
    plt.axvline(mean_correct_confusion, color='k', linestyle='-', linewidth=3)
    plt.axvline(mean_wrong_confusion, color='b', linestyle='--', linewidth=3)
    plt.xlabel(title)
...
def overlay_multi_histograms(save_path, save_name, hist_value_list, title):
    width1=0.05
    layers = sorted(list(hist_value_list.keys()))
    all_list = [np.array(list(hist_value_list[layer].values())).flatten() for layer in layers]
    #for layer in layers:
        #print("values: ",np.array(list(hist_value_list[layer].values())).flatten())
        #plt.hist(np.array(list(hist_value_list[layer].values())).flatten(),bins=10, label=str(layer))
    #plt.hist([i for i in hist_value_list], bins=25, label=[i for i in label_list])
    plt.hist(all_list, label=[str(layer) for layer in layers])
    plt.xlabel(title)
    plt.ylabel('Number of Instances')
    plt.grid(True)
    plt.legend(loc='upper right')
...
def draw_layer_feature_numbers(save_path, save_name, layer_feature1,layer_feature2, first_label, second_label, title):
    hist_value_list1 = []
    hist_value_list2 = []
    for layer in layer_feature1.keys():
        hist_value_list1.append(len(layer_feature1[layer])*[layer])
        hist_value_list2.append(len(layer_feature2[layer])*[layer])
    hist_value_list1 = [j for i in hist_value_list1 for j in i]
    hist_value_list2 = [j for i in hist_value_list2 for j in i]
    plt.hist([hist_value_list1, hist_value_list2], bins=25, label=[first_label, second_label])
    plt.xlabel(title)
    plt.ylabel('Number of Instances')
    plt.grid(True)
    plt.legend(loc='upper right')
...
def get_confusion_scores(outputs, normalize=None, device='cpu'):
    ...
    ###1 confusion with final output
    for output in outputs:  #output[128, 10]
    ...
    ###2 confusion between adjacent layers - when different
    output_pred = [output.max(1, keepdim=True)[1] for output in outputs]
    for i in range(len(outputs)):  #outputs: list
        if i == 0:
            continue
        changed = output_pred[i]!=output_pred[i-1]
        inter_disagreement = 1 - nn.functional.cosine_similarity(outputs[i], outputs[(i-1)], dim=1)  #(i+1)%7
        #inter_disagreement = nn.functional.pairwise_distance(outputs[i], outputs[(i-1)], p=p)
        inter_disagreement = inter_disagreement.to(device)
        for instance_id in range(outputs[0].size(0)):
            #confusion_scores[instance_id] += inter_disagreement[instance_id] #without condition
            if changed[instance_id][0]:
                confusion_scores[instance_id] += inter_disagreement[instance_id] * 2
    

    ###3 compute max confidence when different predict with last layer
    output_pred = [output.max(1, keepdim=True)[1] for output in outputs]
    for i in range(len(outputs)):  #outputs: list
        confidence, pred = outputs[i].max(1, keepdim=True) 
        cur_disagreement = confidence.to(device)
        changed = output_pred[i]!=output_pred[-1]
        for instance_id in range(outputs[0].size(0)):
            if changed[instance_id][0]:
                confusion_scores[instance_id] += cur_disagreement[instance_id][0] * 6
...
def get_dataset(dataset, batch_size=128, add_trigger=False, attack_type='BadNets', clean_ratio=1.0):
    if dataset == 'cifar10':
        return load_cifar10(batch_size, add_trigger, attack_type, clean_ratio)
    elif dataset == 'tinyimagenet':
        return load_tinyimagenet(batch_size, add_trigger, attack_type, clean_ratio)
    elif dataset == 'gtsrb':
        return load_gtsrb(batch_size, add_trigger, attack_type, clean_ratio)

def load_gtsrb(batch_size, add_trigger=False, attack_type='BadNets', clean_ratio=1.0):
    gtsrb_data = GTSRB(batch_size=batch_size, add_trigger=add_trigger, attack_type=attack_type, clean_ratio=clean_ratio)
    return gtsrb_data

def load_cifar10(batch_size, add_trigger=False, attack_type='BadNets', clean_ratio=1.0):
    cifar10_data = CIFAR10(batch_size=batch_size, add_trigger=add_trigger, attack_type=attack_type, clean_ratio=clean_ratio)
    return cifar10_data

def load_tinyimagenet(batch_size, add_trigger=False, attack_type='BadNets', clean_ratio=1.0):
    tiny_imagenet = TinyImagenet(batch_size=batch_size, add_trigger=add_trigger, attack_type=attack_type, clean_ratio=clean_ratio)
